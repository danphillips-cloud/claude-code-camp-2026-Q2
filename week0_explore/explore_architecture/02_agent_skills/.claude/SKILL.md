...
description: >-
  ...
  into, or work toward longer-term objectives in a MUD (tbaMUD/CircleMUD/DikuMUD
  on localhost:4000 or other host:port). Trigger when the user gives a goal
  like "reach level 7", "defeat the goblin king", "map the forest", or just
  says "play the MUD" — the skill will autonomously work toward the goal using
  memory from previous sessions (player stats, world layout, completed tasks).
  Also use for one-off commands, exploration, or NPC interaction.
---

# MUD Player — Goal-Driven Autonomous Play

Play a MUD with persistent goal tracking, autonomous decision-making, and
cross-session memory. The skill reads your current state and world knowledge
...
```
week0_explore/explore_architecture/02_agent_skills/.claude/skills/mud-player/data/player.md
week0_explore/explore_architecture/02_agent_skills/.claude/skills/mud-player/data/world.md
```

The `data/` folder lives inside the mud-player skill directory. When you `cd` into
`.claude/skills/mud-player/` to run `scripts/mud.py`, use the relative path `data/player.md`
or `data/world.md`. From agent code outside the skill directory, use the full paths above.
Read and write these same paths every time so state actually persists.
...
- "reach level 7"
- "defeat the goblin king in the forest"
- "map the entire newbie zone"
- "collect 10 gold coins"

The skill will read the goal, understand what it takes, and autonomously work toward it.
...
These are the verified working credentials for the camp MUD. (`player` is *not*
a registered character — it starts new-character creation. Use `dummy`.)
Override any of them with env vars if the target changes: `MUD_HOST`,
`MUD_PORT`, `MUD_USER`, `MUD_PASSWORD`.

## Commands

`scripts/mud.py` sits next to this file. Run it with the skill directory as the
...
```bash
cd .../02_agent_skills/.claude/skills/mud-player
...
**Defeat/collect goals** ("defeat the goblin king"):
- Search world.md for location hints (e.g., "goblin king seen in forest")
- Navigate there; if location unknown, explore systematically
...
- Stuck? Report blocker to user ("need key to goblin fortress") and pause
...
(Paths are in "Memory file locations" above — always the `02_agent_skills/data/`
copies, never a `data/` folder inside the skill.)
...
- **Active Goal** (freeform text, e.g., "reach level 7" or "defeat the goblin king")
