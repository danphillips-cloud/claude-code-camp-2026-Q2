...
# Runtime files live INSIDE the project (never in machine-local temp), under
# .mud-session/ at the project root. That folder is gitignored, so session state
# stays local to this checkout but is never committed. scripts/ is one level
# below the project root. Override the location with MUD_SESSION_DIR if needed.
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SESSION_DIR = os.environ.get(
    "MUD_SESSION_DIR", os.path.join(_PROJECT_ROOT, ".mud-session")
)
