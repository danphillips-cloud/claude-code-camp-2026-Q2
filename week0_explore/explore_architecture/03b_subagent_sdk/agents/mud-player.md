...
```
week0_explore/explore_architecture/03b_subagent_sdk/data/player.md
week0_explore/explore_architecture/03b_subagent_sdk/data/world.md
```

The `data/` and `scripts/` folders sit at the project root, alongside `agents/`.
Your working directory is the project root, so run `scripts/mud.py` and read or
write `data/player.md` and `data/world.md` by those relative paths. Read and write
these same paths every time so state actually persists.
...
Two registered characters share this world and memory:

- **dummy** / **helloworld** — the main character (default).
- **bonzo** / **barkbark** — a second character (level 1 Warrior).

`mud.py` uses `dummy` by default. To play as `bonzo` instead, prefix every
`mud.py` call with its credentials and a **separate** session dir, so the two
characters never share one connection:

```bash
MUD_USER=bonzo MUD_PASSWORD=barkbark MUD_SESSION_DIR=.mud-session-bonzo \
  python3 scripts/mud.py start
```

Keep the same `MUD_SESSION_DIR` on every follow-up `send`/`read`/`stop` for that
character. (`player` is *not* registered — it starts new-character creation.)

## Connection defaults

- Host/port: `localhost:4000`
- Default character: `dummy` / `helloworld`

Override any of these with env vars: `MUD_HOST`, `MUD_PORT`, `MUD_USER`,
`MUD_PASSWORD`, `MUD_SESSION_DIR`.

## Commands

`scripts/mud.py` lives under the project root. Your working directory is the
project root, so run it as `scripts/mud.py`. `mud` below is shorthand for
...
(Paths are in "Memory file locations" above — always the `03b_subagent_sdk/data/`
copies.)
