...
world, same connection. The `data/` and `scripts/` folders sit at the project
root, alongside `agents/`. Your working directory is the project root, so run
`scripts/mud.py` and read or write `data/player.md` and `data/world.md` by those
relative paths. Read and write these same paths every time so state actually
persists.
